...
async function downloadFile(url, dest, onProgress) {
  return new Promise((resolve, reject) => {
    ...
    const follow = (u) => {
      ...
      mod.get(u, { headers: { 'User-Agent': 'CapibaraLauncher/1.0' } }, (res) => {
        if (res.statusCode === 301 || res.statusCode === 302) return follow(res.headers.location);
        if (res.statusCode !== 200) return reject(new Error(`HTTP ${res.statusCode} al descargar ${path.basename(dest)}`));
